...
test.describe('H. Hybrid Continuity & Order Management (config-off complements)', {
  ...
  test('H1 - PWA Kit <-> SFRA session/basket continuity is correctly absent', {
    tag: '@smoke',
  }, async ({ request }) => {
  ...
  test('H2 - Shipment tracking, cancellation, and returns have nothing to act on (OMS not connected)', {
    ...
    await test.step('OMS metadata is reachable but this order still has no tracking to act on', async () => {
      const oms = await clients.orders(request).getOmsMetadata(workerAccount.accessToken);
      expect([200, 409], `OMS metadata status ${oms.status}`).toContain(oms.status);
      if (oms.status === 409) {
