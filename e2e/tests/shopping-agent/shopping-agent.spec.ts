...
import {
  agentResultCoverageGapDescription,
  commerceQuestion,
...
/*
 * Authored-but-unproven: agent conversation locators target documented Embedded Messaging surface,
 * but cannot be observed on this deployment because app.commerceAgent.enabled is "false".
 * This happy path runs only on a storefront configured for Commerce Agent.
 *
 * Pain-point failures for rows 2 (Agent session fails to initialize) and 3 (Token/context bridge
 * fails) are provider/platform behaviour, outside this storefront contract. Human escalation is
 * not covered.
 */
test('CUJ 8 — obtains actionable commerce assistance without losing storefront context', async ({
  ...
  test.info().annotations.push({
    type: 'coverage-gap',
    description: agentResultCoverageGapDescription,
  });
  ...
  await test.step('Ask commerce question', async () => {
    await Actions.askCommerceQuestion(page, commerceQuestion);
    await expect(Locators.submittedConversationQuestion(page, commerceQuestion)).toBeVisible();
  });

  await test.step('Receive/use result', async () => {
    await expect(Locators.agentResponse(page)).toBeVisible();
  });

  await test.step('Continue shopping action', async () => {
    await Actions.continueShoppingInCart(page);
    await expect(page).toHaveURL(buildPath('/cart'));
  });
});
