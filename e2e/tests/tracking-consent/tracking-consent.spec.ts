...
// This is the one journey the consent form must not arrive pre-answered for, so it
// takes Playwright's own fixtures rather than the shared ones: those set the
// preference cookie up front precisely so the form never interrupts anything else.
...
/**
 * Reach the start of the journey: a shopper on the storefront with no explicit DNT
 * preference, being asked for one. A storefront that renders without ever asking
 * has not retained the tracking-consent UX, which is half of this journey's
 * condition and the half only the rendered page can answer.
 */
...
// The form's own wording is the template's placeholder copy rather than a privacy
// notice, so what is asserted here is that the choice is offered and explained,
// not which words explain it. Wording a merchant must replace before launch would
// otherwise read as a test failure.
...
// CUJ 24 — Set tracking consent across Commerce and analytics systems, taking the
// accept branch: the shopper allows behavioural tracking, the preference is
// written, the SLAS session is reauthorized to carry it, and the effective DNT
// reaches both analytics layers (SLAS + Einstein + Data Cloud).
//
// Conditional journey: it only exists where the consent UX is retained and there
// are analytics layers for the preference to reach, so both halves of the
// condition are proven — the layers from the app's own shipped configuration
// before the browser starts, the UX from the rendered page.
test('a shopper who accepts tracking is tracked by Einstein and Data Cloud', async ({
  ...
  // reauthorized to carry it, and only once both agree is the choice in effect
  // rather than a preference the storefront is about to discard.
...
// rather than only the pop-up closing (SLAS + Einstein + Data Cloud).
test('a shopper who declines tracking is not tracked by Einstein or Data Cloud', async ({
  ...
  // honoured rather than a page that never got there.
